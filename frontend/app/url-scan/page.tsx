...
export default function UrlScanner() {
  ...
  const handleScan = async (scenario: "threat" | "safe" = "threat") => {
    ...
    try {
      ...
      // Parse body first so we can read backend error detail on non-2xx
      ...
      // Backend UrlResponse shape:
      // { risk_score: float, classification: string, original_url: string,
      //   resolved_url: string, highlighted_words: {word, score}[] }
      const typed = data as {
        risk_score: number;
      ...
      // Map backend classification prefix → frontend riskLevel enum
      // "High Risk: ..."    → "Critical"
      // "Medium Risk: ..."  → "Medium"
      // "Low Risk: ..."     → "Low"
      const riskLevel: ThreatResult["riskLevel"] = typed.classification.startsWith("High")
        ? "Critical"
        : typed.classification.startsWith("Medium")
        ? "Medium"
        : typed.classification.startsWith("Low")
        ? "Low"
        : "None";

      setResult({
        threatType: typed.classification,
        riskLevel,
        confidence: Math.round(typed.risk_score * 100),
        explanations: typed.highlighted_words,
        recommendation: typed.classification,
        originalUrl: typed.original_url,
        resolvedUrl: typed.resolved_url,
      });
    } catch (err) {
      const detail =
        err instanceof Error ? err.message : "Failed to scan URL. Please try again.";
      console.error("URL scan failed:", detail);
      setResult({
        threatType: "Scan Failed",
        riskLevel: "None",
        confidence: 0,
        explanations: [{ word: detail, score: 0 }],
        recommendation: "Check the URL is reachable and try again.",
      });
    } finally {
            ...
            <div className="relative bg-black rounded-lg overflow-hidden border border-transparent focus-within:border-white/20 transition-all duration-300">
              ...
              <div className="flex items-center py-6 px-4 md:px-8">
                ...
                <input
                  ...
                  onPaste={(e) => {
                    // Explicitly handle paste to ensure it always works
        ...
        <div className={`transition-all duration-700 ease-out origin-top ${result ? "opacity-100 scale-y-100 translate-y-0 mt-8" : "opacity-0 scale-y-95 translate-y-4 pointer-events-none h-0"}`}>
          {result && (
            <div className="relative">
              ...
              {/* URL resolution trace — only shown when a shortener was expanded */}
